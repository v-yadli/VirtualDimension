...
bool TrayIconsManager::AddIcon(TrayIconHandler* handler)
{
   ...
   _tcscpy_s(data.szTip, handler->GetText());
...
void TrayIconsManager::TrayIconHandler::DisplayBalloon(LPTSTR message, LPTSTR title, UINT timeout, DWORD flags)
{
   ...
   data.hIcon = GetIcon();
   _tcscpy_s(data.szTip, GetText());
   _tcsncpy_s(data.szInfo, message ? message : TEXT(""), _countof(data.szInfo));
   _tcsncpy_s(data.szInfoTitle, title ? title : TEXT(""), _countof(data.szInfoTitle));
