...
const TCHAR Settings::regKeyName[] = TEXT("Software\\Typz Software\\Virtual Dimension\\");

static const RECT DefaultWindowPosition = {10, 10, 110, 110};
static const LOGFONT DefaultPreviewWindowFont = {-12/*height*/,0,0,0,FW_BOLD/*weight*/,FALSE/*italic*/,0,0,DEFAULT_CHARSET,0,0,0,0,TEXT("Arial")/*fontname*/};
static const LOGFONT DefaultOSDFont = {-29/*height*/,0,0,0,FW_BOLD/*weight*/,TRUE/*italic*/,0,0,DEFAULT_CHARSET,0,0,0,0,TEXT("Arial")/*fontname*/};
...
void Settings::InitLocalizedDefault()
{
	// Mod Localize
	// get localized font name
	String osdfont = Locale::GetInstance().GetString(IDS_OSD_FONT_NAME_DEFAULT);
	if (osdfont != TEXT("")) {
		_tcsncpy_s(OSDFont.m_default.lfFaceName, LF_FACESIZE, osdfont, _TRUNCATE);
		//OSDFont.m_default.lfFaceName[LF_FACESIZE-1] = TEXT('\0');
	}
	String previewfont = Locale::GetInstance().GetString(IDS_PREVIEW_FONT_NAME_DEFAULT);
	if (previewfont != TEXT("")) {
		_tcsncpy_s(PreviewWindowFont.m_default.lfFaceName, LF_FACESIZE, previewfont, _TRUNCATE);
		//PreviewWindowFont.m_default.lfFaceName[LF_FACESIZE-1] = TEXT('\0');
	}

}

DWORD Settings::LoadDWord(HKEY regKey, bool keyOpened, const TCHAR * entry, DWORD defVal)
{
   DWORD size;
   DWORD val;
...
void Settings::SaveDWord(HKEY regKey, bool keyOpened, const TCHAR * entry, DWORD value)
{
   if (keyOpened)
      RegSetValueEx(regKey, entry, 0, REG_DWORD, (LPBYTE)&value, sizeof(value));
}

bool Settings::LoadBinary(HKEY regKey, bool keyOpened, const TCHAR * entry, LPBYTE buffer, DWORD length)
{
   DWORD size;
...
void Settings::SaveBinary(HKEY regKey, bool keyOpened, const TCHAR * entry, LPBYTE buffer, DWORD length)
{
   if (keyOpened)
      RegSetValueEx(regKey, entry, 0, REG_BINARY, buffer, length);
}

const TCHAR Settings::regKeyWindowsStartup[] = TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Run\\");
const TCHAR Settings::regValStartWithWindows[] = TEXT("Virtual Dimension");
...
const TCHAR Settings::regSubKeyDisableShellIntegration[] = TEXT("DisableShellIntegration");
const TCHAR Settings::regSubKeyHidingMethods[] = TEXT("HidingMethodsTweaks");

bool Settings::LoadDisableShellIntegration(const TCHAR * windowclass)
{
   HKEY regKey=NULL;
   bool res;
...
void Settings::SaveDisableShellIntegration(const TCHAR * windowclass, bool enable)
{
   HKEY regKey=NULL;
...
int Settings::LoadHidingMethod(const TCHAR * windowclass)
{
   HKEY regKey=NULL;
   DWORD val;
...
void Settings::SaveHidingMethod(const TCHAR * windowclass, int method)
{
   HKEY regKey=NULL;
...
const TCHAR Settings::Desktop::regKeyDesktops[] = TEXT("Desktops");

DEFINE_SETTING(Settings::Desktop, DeskIndex, int, 0);
DEFINE_SETTING(Settings::Desktop, DeskWallpaper, TCHAR*, TEXT(""));
...
Settings::SubkeyList::SubkeyList(Settings * settings, const TCHAR regKey[]): m_group(*settings, regKey)
{
   *m_name = 0;
}

Settings::SubkeyList::SubkeyList(Settings * settings, const TCHAR regKey[], int index): m_group(*settings, regKey)
{
   Open(index);
}

Settings::SubkeyList::SubkeyList(Settings * settings, const TCHAR regKey[], TCHAR * name, bool create): m_group(*settings, regKey)
{
   Open(name, create);
}

bool Settings::SubkeyList::Open(const TCHAR * name, bool create)
{
...
TCHAR * Settings::SubkeyList::GetName(TCHAR * buffer, unsigned int length)
{
	if (m_opened && (buffer != NULL))
		_tcsncpy_s(buffer, length/sizeof(TCHAR), m_name, _TRUNCATE);

	return buffer;
}

bool Settings::SubkeyList::Rename(TCHAR * buffer)
{
   HKEY newKey;
...
const TCHAR Settings::Window::regKeyWindows[] = TEXT("Windows");
