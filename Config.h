...
{
public:
   //Setting(T defval, TCHAR * name): m_default(defval), m_name(name) { }
   //Setting(const T* defval, TCHAR * name): m_default(*defval), m_name(name) { }
   Setting(T defval, char * name):
	   m_default(defval)
   {
#ifdef UNICODE
	   int count = MultiByteToWideChar(CP_ACP, 0, name, -1, NULL, 0);
	   if (count == 0) {
		   return;
	   }
	   m_name = new TCHAR[count];
	   if (MultiByteToWideChar(CP_ACP, 0, name, (int)strlen(name)+1, m_name, count) == 0) {
		   delete m_name;
		   m_name = NULL;
	   }
#else 
   ...
   Setting(const T* defval, char * name) :
	   m_default(*defval)
   {
#ifdef UNICODE
	   int count = MultiByteToWideChar(CP_ACP, 0, name, -1, NULL, 0);
...
public:
   ...
   // Group selection
   virtual bool Open(const TCHAR * path, bool create=true) = 0;
   virtual void Close() = 0;
   bool IsOpened()      { return m_opened; }

   virtual Group * GetSubGroup(const TCHAR * path) = 0;
   ...
   virtual DWORD LoadDWord(const TCHAR * entry, DWORD defVal) = 0;
   virtual void SaveDWord(const TCHAR * entry, DWORD value) = 0;
   virtual bool LoadBinary(const TCHAR * entry, LPBYTE buffer, DWORD length, LPBYTE defval) = 0;
   virtual void SaveBinary(const TCHAR * entry, LPBYTE buffer, DWORD length) = 0;
   virtual unsigned int LoadString(const TCHAR * entry, LPTSTR buffer) = 0;
   virtual void SaveString(const TCHAR * entry, LPTSTR buffer) = 0;
...
public:
   RegistryGroup()                                                                  { }
   RegistryGroup(const TCHAR * path, bool create=true)                               { Open(HKEY_CURRENT_USER, path, create); }
   RegistryGroup(RegistryGroup & parent, const TCHAR * relpath, bool create=true)    { Open(parent.m_regKey, relpath, create); }
   ...
   virtual bool Open(const TCHAR * path, bool create=true)                              { return Open(HKEY_CURRENT_USER, path, create); }
   virtual bool Open(RegistryGroup & parent, const TCHAR * relpath, bool create=true)   { return Open(parent.m_regKey, relpath, create); }
   bool Open(HKEY parent, const TCHAR * path, bool create=true);

   virtual Group * GetSubGroup(const TCHAR * path);

   operator HKEY()                                               { return m_regKey; }

   // Load level accessors
   virtual DWORD LoadDWord(const TCHAR * entry, DWORD defVal);
   virtual void SaveDWord(const TCHAR * entry, DWORD value);
   virtual bool LoadBinary(const TCHAR * entry, LPBYTE buffer, DWORD length, LPBYTE defval);
   virtual void SaveBinary(const TCHAR * entry, LPBYTE buffer, DWORD length);
   virtual unsigned int LoadString(const TCHAR * entry, LPTSTR buffer);
   virtual void SaveString(const TCHAR * entry, LPTSTR buffer);
