...
{
   TRACE(TEXT("~WindowsManager - started"));
   ...
   TRACE(TEXT("~WindowsManager - ended"));
}
   ...
   if (!window)
   {
   ...
   else if (window->CheckCreated())
	{
      //TODO: Need to do better reporting in this case
		TRACE(TEXT("VD puzzled: existing window created !!!"));
	}
}
...
void WindowsManager::CancelDelayedUpdate(Window * win)
{
   ...
   {
      unsigned int idx = (UINT)distance(m_delayedUpdateWndTab.begin(), it);
...
unsigned int WindowsManager::AddDelayedUpdateWnd(Window * wnd)
{
   ...
   {
      idx = (UINT)m_delayedUpdateWndTab.size();
