...
#ifdef _WIN64
#define STACKPTR(Context)  (Context.Rsp)
#else
#define STACKPTR(Context)  (Context.Esp)
#endif

static PVOID AllocProcessMemory(HANDLE hProcess, DWORD_PTR dwNumBytes) {
	CONTEXT Context;
	DWORD dwThreadId;
	DWORD_PTR dwNumBytesXferred;
	HANDLE hThread;
	...
	if (!GetThreadContext(hThread, &Context))
   {
	...
	if (sizeof(mbi) != VirtualQueryEx(hProcess, (PDWORD_PTR)STACKPTR(Context) - 1, &mbi, sizeof(mbi)))
   {
      ResumeThread(hThread);  //let the thread run and vanish
      return NULL;
   }
...
{
	const INT_PTR cbCodeSize = ((LPBYTE)AfterRemoteStartup - (LPBYTE)RemoteStartup);
	const DWORD_PTR cbMemSize = cbCodeSize + sizeof(RemoteStartupArgs) + 3;
   HANDLE hProcess;
...
#ifdef UNICODE
                          GetProcAddress(hinstKrnl, "LoadLibraryW");
#else
...
{
	const INT_PTR cbCodeSize = ((LPBYTE)AfterRemoteCleanup - (LPBYTE)RemoteCleanup);
	const DWORD_PTR cbMemSize = cbCodeSize + sizeof(RemoteCleanupArgs) + 3;
   HANDLE hProcess;
	HINSTANCE hinstKrnl = GetModuleHandle(TEXT("Kernel32"));
	PDWORD pdwCodeRemote = NULL;
