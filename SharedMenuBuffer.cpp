...
bool SharedMenuBuffer::InsertMenu(UINT_PTR id, LPCTSTR text, BOOL check)
{
   ...
   ItemType(m_pBuffer) = (TCHAR)(check ? MENUITEM_CHECKED : MENUITEM_UNCHECKED);
   ItemId(m_pBuffer) = (UINT)id;
   _tcscpy_s(ItemName(m_pBuffer), sizeof(ItemName(m_pBuffer))/sizeof(TCHAR), text ? text : TEXT(""));
