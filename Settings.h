...
public:
   ...
   // Mod Localize
   static void InitLocalizedDefault();
   ...
   bool LoadDisableShellIntegration(const TCHAR * windowclass);
   void SaveDisableShellIntegration(const TCHAR * windowclass, bool enable);
   int LoadHidingMethod(const TCHAR * windowclass);
   void SaveHidingMethod(const TCHAR * windowclass, int method);

   class SubkeyList: public Config::RegistryGroup
   {
   public:
      SubkeyList(Settings * settings, const TCHAR regKey[]);
	  SubkeyList(Settings * settings, const TCHAR regKey[], int index);
	  SubkeyList(Settings * settings, const TCHAR regKey[], TCHAR * name, bool create = true);

      virtual bool Open(int index);
      virtual bool Open(const TCHAR * name, bool create=true);

      bool IsValid();
      void Destroy();

      TCHAR * GetName(TCHAR * buffer, unsigned int length);
      bool Rename(TCHAR * buffer);
   ...
   public:
      ...
      Desktop(Settings * settings, TCHAR * name): SubkeyList(settings, regKeyDesktops, name)  {}
   ...
   protected:
      static const TCHAR regKeyDesktops[];
   };
   ...
   public:
      ...
      Window(Settings * settings, TCHAR * name, bool create=false): SubkeyList(settings, regKeyWindows, name, create)  {}

      bool OpenDefault()                                       { return Open(NULL); }
      virtual bool Open(const TCHAR * name, bool create=false)  { return SubkeyList::Open(name, create); }
   ...
   protected:
      static const TCHAR regKeyWindows[];
   };

protected:
   static const TCHAR regKeyName[];
   static const TCHAR regKeyWindowsStartup[];

   static const TCHAR regSubKeyDisableShellIntegration[];
   static const TCHAR regSubKeyHidingMethods[];
   static const TCHAR regValStartWithWindows[];

   static DWORD LoadDWord(HKEY regKey, bool keyOpened, const TCHAR * entry, DWORD defVal);
   static void SaveDWord(HKEY regKey, bool keyOpened, const TCHAR * entry, DWORD value);
   static bool LoadBinary(HKEY regKey, bool keyOpened, const TCHAR * entry, LPBYTE buffer, DWORD length);
   static void SaveBinary(HKEY regKey, bool keyOpened, const TCHAR * entry, LPBYTE buffer, DWORD length);
