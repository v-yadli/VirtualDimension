...
void OnScreenDisplayWnd::Create()
{
   ...
   m_hWnd = CreateWindowEx( WS_EX_TOPMOST | (m_isTransparent ? WS_EX_TRANSPARENT : 0) | WS_EX_TOOLWINDOW, 
                            (LPTSTR)MAKEINTRESOURCE(s_classAtom), TEXT("OSD"), WS_POPUP, 
...
void OnScreenDisplayWnd::Display(TCHAR * str, int timeout)
{
   SIZE size;
   HFONT defFont;
   HDC hdc;
   ...
   if (str != m_text)
	   _tcsncpy_s(m_text, _countof(m_text), str, _TRUNCATE);
