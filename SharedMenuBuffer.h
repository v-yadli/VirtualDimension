...
#define ITEMTYPE_LEN    sizeof(TCHAR)
...
protected:
   ...
   TCHAR& ItemType(TCHAR * buffer)       { return *buffer; }
   UINT& ItemId(TCHAR * buffer)     { return *(UINT*)(buffer+ITEMTYPE_LEN); }
   TCHAR* ItemName(TCHAR * buffer)       { return buffer+ITEMTYPE_LEN+ITEMID_LEN; }
   size_t ItemLength(TCHAR * buffer)       { return _tcslen(ItemName(buffer)) + 1 + ITEMTYPE_LEN + ITEMID_LEN; }
