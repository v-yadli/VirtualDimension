...
public:
    ...
    */
   CommandLineOption(TCHAR opcode, UINT resid, ArgType arg = no_argument);
...
public:
   CommandLineFlag(TCHAR opcode, UINT resid):
...
public:
   CommandLineInt(TCHAR opcode, UINT resid, int defval=0, int optval=1, ArgType type=required_argument):
...
public:
   CommandLineStr(TCHAR opcode, UINT resid, const TCHAR* defval=TEXT(""), const TCHAR* optval=TEXT(""), ArgType type=required_argument):
      CommandLineOption(opcode, resid, type), m_str(defval), m_optval(optval)
   {}

   virtual void ParseOption(const TCHAR* arg)	{ m_str = arg ? arg : m_optval; }
