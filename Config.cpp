...
unsigned int Group::LoadSetting(const Setting<LPTSTR> &setting, LPTSTR buffer, unsigned int length, const StringSetting& /*type*/)
{
   ...
   {
      size = _tcslen(setting.m_default);
	  _tcsncpy_s(buffer, length, setting.m_default, length - 1);
      //buffer[length-1] = 0;
   }
...
bool RegistryGroup::Open(HKEY parent, const TCHAR * path, bool create)
{
   if (m_opened)
      Close();
...
Group * RegistryGroup::GetSubGroup(const TCHAR * path)
{
...
DWORD RegistryGroup::LoadDWord(const TCHAR * entry, DWORD defVal)
{
   DWORD size;
   DWORD val;
   DWORD type;
...
void RegistryGroup::SaveDWord(const TCHAR * entry, DWORD value)
{
   if (m_opened)
      RegSetValueEx(m_regKey, entry, 0, REG_DWORD, (LPBYTE)&value, sizeof(value));
}

bool RegistryGroup::LoadBinary(const TCHAR * entry, LPBYTE buffer, DWORD length, LPBYTE defval)
{
   DWORD size;
   DWORD type;
   bool res;
...
void RegistryGroup::SaveBinary(const TCHAR * entry, LPBYTE buffer, DWORD length)
{
   if (m_opened)
      RegSetValueEx(m_regKey, entry, 0, REG_BINARY, buffer, length);
}

unsigned int RegistryGroup::LoadString(const TCHAR * entry, LPTSTR buffer)
{
   DWORD size;
   DWORD type;
...
void RegistryGroup::SaveString(const TCHAR * entry, LPTSTR buffer)
{
   DWORD len;
