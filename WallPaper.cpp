...
bool WallPaper::RefreshDefaultWallpaper()
{
	TCHAR prevWallpaper[MAX_PATH];
	if (m_defaultWallpaperInit)
		_tcscpy(prevWallpaper, m_defaultWallpaper);
	else
		*prevWallpaper = 0;

	m_defaultWallpaperInit = false;
	LoadDefaultWallpaper();

	return _tcscmp(prevWallpaper, m_defaultWallpaper) == 0;
}
   ...
   if (!m_defaultWallpaperInit)
   {
...
#endif
	  {
         ...
         SystemParametersInfo(SPI_GETDESKWALLPAPER, _countof(m_defaultWallpaper), m_defaultWallpaper, 0);
		  ...
		  _tcsncpy_s(m_defaultWallpaper, _countof(m_defaultWallpaper), buffer, _TRUNCATE);
	  }
#endif
      ...
      ReleaseMutex(self->m_hQueueMutex);
		...
		if (changed || !wallpaper->m_useDefaultWallpaper)
		{
		   //TODO: for default wallpaper, we should now check if it has not changed
			...
			{
				if (_tcsncicmp(wallpaper->m_fileName + _tcslen(wallpaper->m_fileName) - 4, TEXT(".bmp"), 4) == 0)
				{
